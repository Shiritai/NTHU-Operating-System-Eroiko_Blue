...
int OpenFile::WriteAt(char *from, int numBytes, int position)
{
    ...
    // cout << "[OpenFile::WriteAt] firstSector: " << firstSector << ", lastSector: " << lastSector << endl;
    for (i = firstSector; i <= lastSector; i++) {
        int secNum = hdr->ByteToSector(i * SectorSize);
        // cout << "Write sector: " << secNum << endl;
        kernel->synchDisk->WriteSector(
            secNum, &buf[(i - firstSector) * SectorSize]);
