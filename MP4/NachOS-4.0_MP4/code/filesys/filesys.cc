...
// ----------------------------------------------------------------------
//+ MP4
// The file size of FreeMap is NumSectors/BitsInByte bytes
// because we mark a sector as used/unused with a bit,
// and there are NumSectors of sectors, so we need NumSectors bits,
// i.e. NumSectors/BitsInByte bytes!
// ----------------------------------------------------------------------
...
FileSystem::FileSystem(bool format)
{
    ...
    if (format)
    {
        ...
        if (debug->IsEnabled(dbgFile)) {
            cout << "Print freeMapFile and directoryFile " 
                << "structure of formatted FileSystem..." << endl;
            mapHdr->Print();
            dirHdr->Print();
        }
...
//----------------------------------------------------------------------
// FileSystem::CreateFileDir
// 	Create a file or directory in the Nachos file system (similar to UNIX Mkdir).
...
//	to the file system!
//
//	"path" -- path of directory to be create
//	"isDIr" -- whether we're creating a directory or a file
//  "initialSize" -- size of "file" to be created, 
//      if is directory, this parameter will be ignored
//----------------------------------------------------------------------
bool FileSystem::CreateFileDir(const char * path, bool isDir, int initialSize)
{
    const char * attr = isDir ? "directory" : "file"; // for debugging
    ...
    // allocate corresponding file header with correct size
...
//----------------------------------------------------------------------
// FileSystem::OpenAsOpenFile
// 	Open a file for reading and writing, 
//      should manager the file by yourself.
//
//	"path" -- the text name of the file to be opened
//----------------------------------------------------------------------
...
//	in the file system.
//
//	"path" -- the text name of the file to be removed
//----------------------------------------------------------------------
...
//----------------------------------------------------------------------
// static FileSystem::RemoveSingleObj
// 	Delete a object (file/directory) from the file system.  
//
//  Notice that the caller should manager the directory 
//      that contains this object!
//  
//	"sector" -- the sector of the file to be removed
//	"freeMap" -- the corresponding freeMap object
//----------------------------------------------------------------------
void FileSystem::RemoveSingleObj(int sector, PersistentBitmap * freeMap)
{
...
// 	Delete all file/directory from the file system.  This requires:
//	    Remove it from the directory
//	    Delete the space for its header
//	    Delete the space for its data blocks
//	    Write changes to directory, bitmap back to disk
...
{
    string curDir(path);
    if (*curDir.rbegin() != '/')  // /PATH_OF_DIR -> /PATH_OF_DIR/.
        curDir += "/.";
    else // /PATH_OF_DIR/ -> /PATH_OF_DIR/.
        curDir += ".";
...
{
    DEBUG(dbgFile, "Start traversing path: " << path);
    const char * attr = isDir ? "directory" : "file"; // for debugging

    // parse path
    vector<string> path_list = ParsePath(path);
    if (debug->IsEnabled(dbgFile)) {
        cout << "Parsing as " << attr 
            << " with path: [" << path_list.front() << ']';
