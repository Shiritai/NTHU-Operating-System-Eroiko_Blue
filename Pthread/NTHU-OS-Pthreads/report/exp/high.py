...
aver=[]
data_col=[]
with open("high_threshold.log") as f:
    file=f.readlines()
    for line in file:
...
data=np.array(final)
with open("high_threshold.txt", "w+") as f:
    # data = f.read()
...
# plt.title("high_threshold")
# plt.xlabel(data_col[0])
# plt.ylabel(data_col[1])
# plt.plot(x,y)
# # plt.show()
# plt.savefig("high_threshold_vs_time")
...
plt.legend(loc=2)
# plt.show()
plt.savefig("high_threshold_cscl")
