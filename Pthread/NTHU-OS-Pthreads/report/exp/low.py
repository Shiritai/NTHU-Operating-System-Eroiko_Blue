...
aver=[]
data_col=[]
with open("low_threshold.log") as f:
    file=f.readlines()
    for line in file:
...
data=np.array(final)
with open("low_threshold.txt", "w+") as f:
    # data = f.read()
...
# plt.title("low_threshold")
# plt.xlabel(data_col[0])
# plt.ylabel(data_col[1])
# plt.plot(x,y)
# # plt.show()
# plt.savefig("low_threshold_vs_time")
...
plt.legend(loc=2)
# plt.show()
plt.savefig("low_threshold_cscl")
